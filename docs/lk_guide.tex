...
\begin{verbatim}
const local show_welcome = define( <args> )  {  <statements>  };
\end{verbatim}

The meaning of the \texttt{local} keyword will be explained in the next section.  The alternate syntax described here can help clarify programs and is generally easier for people familiar with other languages, at least initially.  It automatically enforces \emph{const-ness}, so that the function isn't inadvertently replaced later in the code by an assignment statement.
