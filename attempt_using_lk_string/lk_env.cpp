...
bool lk::vardata_t::as_boolean()
{
	...
	if (m_type == STRING)
	{
		...
		for( size_t i=0;i<slower.length();i++ )
			slower[i] = (lk_char) tolower( (lk_char)slower[i] );
...
lk_string lk::vardata_t::as_string()
{
	switch (m_type)
	{
...
const char *lk::vardata_t::typestr()
{
	switch(m_type)
	{
	case NULLVAL: return "null";
...
lk_string &lk::vardata_t::str() throw(error_t)
{
...
lk::vardata_t *lk::vardata_t::lookup( const lk_string &key ) throw(error_t)
{
	if (m_type != HASH) throw error_t("access violation to non-hash data");
	register varhash_t &h = *reinterpret_cast< varhash_t* >(m_u.p);
	varhash_t::iterator it = h.find( key );
