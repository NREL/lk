...
{
public:
	ScriptWindow( wxWindow *parent, int id = wxID_ANY, const wxPoint &pos = wxDefaultPosition, const wxSize &size = wxSize(800,900) );
...
{
	ScriptWindow *sw = new ScriptWindow( 0, wxID_ANY, wxDefaultPosition, wxSize( 760, 800 ) );
	sw->Show( show );
	return sw;
}
...
{
	wxFileDialog dlg( current, "Open script", 
...
void ScriptWindow::OnCommand( wxCommandEvent &evt )
{
	...
	case wxID_EXECUTE:
		...
		Layout();
		wxSafeYield( 0, true );
		m_script->Execute( wxPathOnly(m_fileName), this );
		m_stopBtn->Hide();
		m_runBtn->Show();
		Layout();
		break;
